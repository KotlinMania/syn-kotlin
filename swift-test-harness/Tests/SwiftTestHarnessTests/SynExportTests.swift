...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Syn.swiftmodule/`
...
//      `import Syn` above would fail at compile time.
//
//   2. The static archive `libSyn.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Syn" }` and
//      `flattenPackage = "io.github.kotlinmania.syn"` configuration in
...
final class SynExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Syn swift module imported cleanly")
    }
}
