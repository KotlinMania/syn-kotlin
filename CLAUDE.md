# Claude Code Project Instructions — syn-kotlin

## Project Overview

This is **syn-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`syn`](https://crates.io/crates/syn).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/syn/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/syn>
