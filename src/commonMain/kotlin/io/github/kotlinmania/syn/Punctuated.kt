...
public class Punctuated<T : ToTokens, P : ToTokens> private constructor(
    ...
    /** Returns a mutable list of pairs for in-place mutation of values and punctuation. */
    ...
    public fun intoPairs(): List<PunctuatedPair<T, P>> {
        ...
        for ((value, punctuation) in inner) {
            result.add(PunctuatedPair.Punctuated(value, punctuation))
        }
        val tail = last
        if (tail != null) {
            result.add(PunctuatedPair.End(tail))
        }
        return result
    }
    ...
    /** Returns an [IntoIter] that owns the values, consuming this sequence. */
    public fun intoIter(): IntoIter<T> {
    ...
    override fun toTokens(tokens: TokenStream) {
        for (pair in pairs()) {
            pair.first.toTokens(tokens)
            pair.second?.toTokens(tokens)
        }
    }
}
...
public sealed class PunctuatedPair<out T : ToTokens, out P : ToTokens> {
    ...
    public fun punct(): P? =
        when (this) {
            is Punctuated -> punctuation
            is End -> null
        }

    public fun intoTuple(): Pair<T, P?> =
        when (this) {
            is Punctuated -> value to punctuation
            is End -> value to null
        }

    public companion object {
        public fun <T : ToTokens, P : ToTokens> new(t: T, p: P?): PunctuatedPair<T, P> =
            if (p != null) Punctuated(t, p) else End(t)
    }
}
...
public class IntoIter<T : ToTokens>(private val elements: MutableList<T>) : Iterator<T> {
    ...
    override fun next(): T =
        if (hasNext()) elements[index++] else throw NoSuchElementException()
...
public class PunctuatedPairs<T : ToTokens, P : ToTokens> internal constructor(
    ...
    private var lastConsumed: Boolean = lastValue == null

    override fun hasNext(): Boolean = inner.hasNext() || !lastConsumed

    override fun next(): Pair<T, P?> {
        if (inner.hasNext()) {
            val pair = inner.next()
