 ...
 *
 * The upstream Rust spelling is `pub(crate) struct TokenTreeHelper<'a>(pub
 * &'a TokenTree)` with `PartialEq` and `Hash` impls. Kotlin's wrapper-struct
 ...
 * Mirrors the upstream `PartialEq` impl on `TokenStreamHelper`.
 */
 ...
 * `Hash` impl on `TokenStreamHelper`.
 */
