...
// ── Delimiter Peek / Parse ────────────────────────────────────────────────

/** Peeks for the `{` delimiter token. */
@HiddenFromObjC
...
/** Parses the `{` delimiter token. */
@HiddenFromObjC
public object BraceParse : Parse<Brace> {
 override fun parse(input: ParseStream): SynResult<Brace> =
  input.step { cursor ->
   val (_, span, after) = cursor.group(Delimiter.Brace)
    ?: return@step SynResult.failure(cursor.error("expected `{`"))
   SynResult.success(Brace.from(span.join()) to after)
  }
}
/** Peeks for the `[` delimiter token. */
@HiddenFromObjC
...
public object BracketParse : Parse<Bracket> {
 override fun parse(input: ParseStream): SynResult<Bracket> =
  input.step { cursor ->
   val (_, span, after) = cursor.group(Delimiter.Bracket)
    ?: return@step SynResult.failure(cursor.error("expected `[`"))
   SynResult.success(Bracket.from(span.join()) to after)
  }
}
/** Parses the `(` delimiter token. */
@HiddenFromObjC
public object ParenParse : Parse<Paren> {
 override fun parse(input: ParseStream): SynResult<Paren> =
  input.step { cursor ->
   val (_, span, after) = cursor.group(Delimiter.Parenthesis)
    ?: return@step SynResult.failure(cursor.error("expected `(`"))
   SynResult.success(Paren.from(span.join()) to after)
  }
}